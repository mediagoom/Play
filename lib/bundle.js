(function (dashjs) {
  ...
  var PACKEVENT = Object.freeze({
    STARTED: "started",
    ERROR: "error"
  });
  var PLAYEVENT = Object.freeze({
    STARTED: "started",
    ERROR: "error",
    PLAYER: "player",
    URL: "url"
  ...
  function (_EventEmitter) {
    _inherits(basepack, _EventEmitter);

    function basepack() {
      _classCallCheck(this, basepack);

      return _possibleConstructorReturn(this, _getPrototypeOf(basepack).apply(this, arguments));
    }

    _createClass(basepack, [{
      key: "raiseEvent",
      value: function raiseEvent(packevent) {
        this.emit(packevent);
      }
    }, {
      key: "events",
  ...
  function dodash(myself, src, id, autoplay) {
    var url = src; //var key = URI(url).absoluteTo('..');
    //var key = URI('./json.ck').absoluteTo(src).toString();

    myself.dashplayer = dashjs.MediaPlayer().create();
    var elem = document.querySelector("#" + id);
    if (null == autoplay) autoplay = false;
    /*
  ...
  /*#__PURE__*/
  function (_basepack) {
    _inherits(dashpack, _basepack);

    function dashpack(options) {
      var _this;

      _classCallCheck(this, dashpack);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(dashpack).call(this));
      _this.dashplayer = null;
      return _this;
    }

    _createClass(dashpack, [{
      key: "play",
      value: function play(url, videotagid, poster, autoplay) {
  ...
  function dovideo(myself, src, id, autoplay) {
    var url = src;
  ...
  /*#__PURE__*/
  function (_basepack) {
    _inherits(videopack, _basepack);

    function videopack(options) {
      var _this;

      _classCallCheck(this, videopack);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(videopack).call(this));
      _this.videoplayer = null;
      return _this;
    }

    _createClass(videopack, [{
      key: "play",
      value: function play(url, videotagid, poster, autoplay) {
        //debugger;
      ...
      key: "time",
      value: function time() {
        var played = this.videoplayer.played;
  ...
  function drm_detect(obj) {
    ...
    if (window['MSMediaKeys'] && window['MSMediaKeys'].isTypeSupported) {
  ...
  function checkPlay(element, mime, strict, context) {
    ...
    if (strict && testElement.canPlayType(mime) === 'probably' || !strict && testElement.canPlayType(mime) !== '') {
      return true;
    }
  ...
  function () {
    ...
    _createClass(detector, [{
      key: "Video5",
      value: function Video5() {
        return !!this._video.canPlayType;
      }
    }, {
      key: "H264",
      value: function H264(profile) {
      ...
      value: function flash() {
        ...
        var ua = this.getUserAgent(); // IE 9, 10 with ActiveXObject (not tested on IE 8)
  ...
  var dbg = browser$1('mgplay:play'); //localStorage.debug = 'mgplay:*'

  function html5embed(src, poster, id, options) {
    var defopt = {
      autoplay: true,
      controls: true,
      cssclass: null,
    ...
    var opt = _extends(defopt, options);
  ...
  function absolute(base, move) {
    var url = URI(move); // url = url.segment(move);

    return url.absoluteTo(base).href();
  }

  function registerevents(myself) {
    myself.player.on(myself.player.events.STARTED, function () {
      return myself.emit(myself.events.STARTED);
    });
  }
  ...
  /*#__PURE__*/
  function (_EventEmitter) {
    _inherits(mgPlayer, _EventEmitter);

    function mgPlayer(options) {
      var _this;

      _classCallCheck(this, mgPlayer);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(mgPlayer).call(this));
      var defopt = {
        autoplay: true,
        id: 'player',
        hostid: 'playerhost',
      ...
      _this.options = _extends(defopt, options);
      _this.tag = null;
      _this.player = null;
      _this.detector = new detector();
      return _this;
    }

    _createClass(mgPlayer, [{
      key: "_loadplayer",
      value: function _loadplayer(player, url, poster) {
        var v5 = html5embed(null, poster, this.options.id, this.options);
        var div = document.getElementById(this.options.hostid);
        div.innerHTML = v5;
        this.player = player;
        registerevents(this);
      ...
      key: "playdash",
      value: function playdash(url, poster) {
        if (!this.detector.MediaExtension()) throw new Error('No MSE support detected');

        this._loadplayer(new dashpack(), url, poster);
      }
    }, {
      key: "playhls",
      value: function playhls(url, poster) {
        if (this.detector.HLSNative()) {
          this._loadplayer(new videopack(), url, poster);

          return;
        } else {
          if (this.detector.MediaExtension()) throw new Error('MSE HLS not supported yet');
          return;
        }
      ...
      key: "play",
      value: function play(infouri, info) {
        /*
        {
        "status":"ok",
        "name":"sample_flv",
  ...
  window.mgPlayer = function (opt) {
    return new mgPlayer(opt);
  };
